...
export function Navigation() {
  ...
  const refreshUnread = () => { if (status === "authenticated") void unreadCount().then(setUnread).catch(() => undefined) }
  useEffect(() => {
  ...
  async function markRead(notification: Notification) {
    ...
    try { await markNotificationRead(notification.id); setRecent((items) => items.map((item) => item.id === notification.id ? { ...item, readAt: new Date().toISOString() } : item)); setUnread((value) => Math.max(0, value - 1)); emitNotificationsUpdated() } catch {}
  }
  async function readAll() {
    try { await markAllNotificationsRead(); setRecent((items) => items.map((item) => item.readAt ? item : { ...item, readAt: new Date().toISOString() })); setUnread(0); emitNotificationsUpdated() } catch {}
  }
  async function handleLogout(event: React.MouseEvent) { event.preventDefault(); await logout(); window.location.href = "/" }
        ...
        <ul className="nav navbar-nav navbar-right">
          {authed ? <>
            ...
            <li className="nav-item nav-meta">
              ...
              {menuOpen ? <div className="nav-notif-panel" onMouseLeave={() => setMenuOpen(false)}>
                {recentLoading ? <div className="nav-notif-item">Загрузка…</div> : recent.length === 0 ? <div className="nav-notif-item nav-notif-empty">Нет уведомлений</div> : recent.map((notification) => <Link key={notification.id} href={notificationLink(notification)} className="nav-notif-item" onClick={() => void markRead(notification)}><div className="nav-notif-title">{notificationLabel(notification)}</div>{notificationMeta(notification) ? <div className="nav-notif-meta">{notificationMeta(notification)}</div> : null}<div className="message-meta">{new Date(notification.createdAt).toLocaleString("ru-RU")}</div></Link>)}
